...
This document is normative for commands that can mutate durable working data, perform external/network/batch activity, call paid APIs/LLMs/routing providers, or otherwise create material external cost/effect.
...
- If temporary DB writes are required internally, they must be isolated/rolled back so working state is unchanged.
- External/network calls are permitted only when the command/work contract explicitly allows them; dry-run does not automatically authorize external cost/effect.
...
Typical evidence progression, when applicable:
...
Not every change requires every stage; the work contract defines relevant evidence.

## Database isolation

The technical foundation must keep automated test state separate from the local working database.

A sandbox may be used for exploratory/integration validation. Neither TEST nor DRY-RUN may silently damage or commit to the user's canonical working data.

## Cost, providers, and secrets

The current project policy defines no universal provider-specific budget or rate limit.
